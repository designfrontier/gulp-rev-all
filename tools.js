...
module.exports = ( function () {
    ...
    // Taken from gulp-rev: https://github.com/sindresorhus/gulp-rev
    var revFile = function (filePath, options) {
    ...
    var revReferencesInFile = function (file, options) {

        var replaceMap = {};
        var rootDir = options.rootDir || path.dirname(file.path);
        ...
        while (result = filepathRegex.exec(contents)) {
            ...
            if (rootDir) {
                fullpath = path.join(rootDir, result[1]);
                if (fs.existsSync(fullpath)) {
                    replaceMap[result[1]] = path.dirname(result[1]) + '/' + path.basename(revFile(fullpath, options));
            ...
            fullpath = path.join(file.base, result[1]);
            if (fs.existsSync(fullpath)) {
                replaceMap[result[1]] = path.dirname(result[1]) + '/' + path.basename(revFile(fullpath, options));
